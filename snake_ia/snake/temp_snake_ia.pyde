...
def save_Q_table():
    ...
    for i in range(len(Q_table)):
        for j in range(len(Q_table[i]) - 1):
            f.write(str(Q_table[i][j]) + ",")
        f.write(str(Q_table[i][len(Q_table[i]) - 1]) + "\n")
    f.close()
...
def draw():
    if run:
        ...
        #print("state: ", st, "at: ", at, "r_pos: ", snake.r_pos)
        #print(Q_table[st])        
        stp1, r = snake.step(at)
        #print(r)
